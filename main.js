...
// funciones de comparacion para que sea cese-insensitive, remplaza espacios vacios y acentos pas aa minusculas
function normalizeHeaderCell(v) {
  if (v === null || typeof v === 'undefined') return '';
  let s = String(v).replace(/^\uFEFF/, '').trim().toLowerCase();
  s = s.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  return s;
}

function arrayFirstRowToHeader(row) {
  return row.map(h => normalizeHeaderCell(h));
}
...
function arraysEqual(a, b) {
  ...
  for (let i = 0; i < a.length; i++) {
    if (normalizeHeaderCell(a[i]) !== normalizeHeaderCell(b[i])) return false;
  }
  return true;
}

function findHeaderRowIndex(jsonRows) {
...
mergeBtn.addEventListener('click', async () => {
  ...
  const header = p.json[headerRowIdx] ? arrayFirstRowToHeader(p.json[headerRowIdx]) : [];
